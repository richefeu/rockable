...
public:
  ...
  void sort_Ij()
  {
	...
	__gnu_parallel::stable_sort(Ij.begin(), Ij.end(), [](std::pair<size_t,size_t> a, std::pair<size_t,size_t> b) { return a.second < b.second; });
  } 

  std::vector<std::pair<size_t,size_t>> Ii; ///< Ii contains I->i for a given active interaction (first = id, second = I->i). This vector is sorted in "accelerations" by construction.
  std::vector<std::pair<size_t,size_t>> Ij; ///< Ij contains I->j for a given active interaction (first = id, second = I->j). This vector is sorted in "accelerations".
  ...
  std::vector<size_t> size_i; ///< Store the number of elements of the same value of Ii.second ie. size_i[id] = number of Elements in Ii.second equals to id.
  std::vector<size_t> size_j; ///< Store the number of elements of the same value of Ij.second ie. size_j[id] = number of Elements in Ij.second equals to id.
