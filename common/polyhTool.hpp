...
public:
  ...
  static void sobolSequence(const int n, std::vector<double> &x) {
    ...
    static unsigned int iv[MAXDIM * MAXBIT] = {1, 1, 1,  1,  1, 1,  3,  1,
                                               3, 3, 1,  1,  5, 7,  7,  3,
                                               3, 5, 15, 11, 5, 15, 13, 9};
    static double fac;
  ...
  static int cut_poly(const poly &block, const plan &pl, poly &sub_block) {
    ...
    std::map<std::pair<int, int>, int>
        intersected_edges; // associate a pair of nodes with a new node number
    std::pair<int, int> node_pair;
    int node0, node1;
    double alpha;
    size_t initial_node_number =
        sub_block.nodes.size(); // The number of nodes before we add those from
                                // the intersections
    ...
    for (size_t e = 0; e < ne; e++) {
      ...
      if (alpha > 0.0 && alpha < 1.0) {
        ...
        if (node_pair.second <
            node_pair.first) { // Take care that first number is lower than the
                               // second one
          tmp = node_pair.first;
    ...
    if (nb_cut < 3) {
      return 0;
    } // Because, we need 3 points (at least) to create the face that close the
      // shape
    ...
    for (size_t f = 0; f < nf; f++) {
      for (it0 = sub_block.faces[f].nodes.begin();
           it0 != sub_block.faces[f].nodes.end(); ++it0) {
        it1 = it0;
        ++it1;
    ...
    for (size_t f = 0; f < sub_block.faces.size(); f++) {
      ...
      E.node0 = E.node1 = -1;
      size_t count = 0;
      for (std::list<int>::iterator it = sub_block.faces[f].nodes.begin();
           it != sub_block.faces[f].nodes.end(); ++it) {
        if (*it >= (int)initial_node_number) { // It means that this is a node
                                               // that has been added
          if (num_search == 0) {
            E.node0 = *it;
            num_search = 1;
    ...
    for (std::list<pedge>::iterator ite = edge_loop0.begin();
         ite != edge_loop0.end(); ++ite) {
    ...
    */
    ...
    for (size_t e = 0; e < sub_block.edges.size(); e++) {
      if (nodes_to_remove.find(sub_block.edges[e].node0) ==
              nodes_to_remove.end() &&
          nodes_to_remove.find(sub_block.edges[e].node1) ==
              nodes_to_remove.end())
...
template <> struct less<polyhTool::pedge> {
  bool operator()(const polyhTool::pedge &lhs,
                  const polyhTool::pedge &rhs) const {
